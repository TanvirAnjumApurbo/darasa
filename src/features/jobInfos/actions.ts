...
export async function deleteJobInfo(id: string) {
  ...
  const deletedJobInfo = await deleteJobInfoDb(id);
  if (deletedJobInfo == null) {
    return {
      error: true,
      message: "Job description could not be deleted",
    };
  }

  const remainingJobInfo = await db.query.JobInfoTable.findFirst({
    where: eq(JobInfoTable.userId, userId),
  });

  if (remainingJobInfo == null) {
    return {
      success: true,
      redirectTo: "/app/job-infos/new",
    };
  }
